...
const UploadPage: React.FC = () => {
  ...
  const validateComponent = (component: any): string[] => {
    const errors: string[] = [];
    
    if (!component.towerName?.trim()) errors.push('Tower Name is required');
    if (!component.appGroup?.trim()) errors.push('App Group is required');
    if (!component.componentType?.trim()) errors.push('Component Type is required');
    if (!component.complexity || !['Simple', 'Medium', 'Complex'].includes(component.complexity)) {
      errors.push('Complexity must be Simple, Medium, or Complex');
    }
    if (!component.status || !['Released', 'In Development', 'Planned'].includes(component.status)) {
      errors.push('Status must be Released, In Development, or Planned');
    }
    if (!component.year || component.year < 2020 || component.year > 2030) {
      errors.push('Year must be between 2020 and 2030');
    }
    if (!component.month || component.month < 1 || component.month > 12) {
  ...
  // Helper function to map complexity values
          ...
          complete: (result) => {
            ...
            result.data.slice(1).forEach((row: any, index) => {
              if (row.length > 1) {
                ...
                const validationErrors = validateComponent(component);

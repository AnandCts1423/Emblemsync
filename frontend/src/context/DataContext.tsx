...
interface DataContextType {
  ...
  batchDeleteComponents: (componentIds: number[]) => Promise<{ success: boolean; message: string; deletedCount?: number; }>;
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const handleComponentUpdate = useCallback((event: CustomEvent) => {
    const { action, component } = event.detail;
  ...
  const batchDeleteComponents = useCallback(async (componentIds: number[]): Promise<{
    success: boolean;
    message: string;
    deletedCount?: number;
  }> => {
    try {
      setError(null);
      const response = await apiService.batchDeleteComponents(componentIds);
      if (response.success && response.data) {
        // Remove deleted components from state
        setComponents(prev => 
          prev.filter(comp => !componentIds.includes(comp.id!))
        );
        return {
          success: true,
          message: response.message || `Successfully deleted ${response.data.deletedCount} components`,
          deletedCount: response.data.deletedCount
        };
      } else {
        setError(response.error || 'Batch delete failed');
        return {
          success: false,
          message: response.error || 'Batch delete failed'
        };
      }
    } catch (err) {
  ...
  // Initialize data on mount and setup real-time listeners
  useEffect(() => {
    refreshData();
    
    // Set up real-time event listeners
    window.addEventListener('component_update', handleComponentUpdate as EventListener);
    
    return () => {
      window.removeEventListener('component_update', handleComponentUpdate as EventListener);
    };
  }, [refreshData, handleComponentUpdate]);
